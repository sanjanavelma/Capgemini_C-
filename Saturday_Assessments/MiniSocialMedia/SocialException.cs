...
namespace MiniSocialMedia
{
    ...
    public partial class User : IPostable, IComparable<User>
    {
        ...
        private readonly HashSet<string> _following = new(StringComaparer.OrginalIngnoreCase);
        public event Action<Post>? OnNewPost;
        public User(string username, string email)
        {
            if (string.IsNullOrWhiteSpace(username))
            {
                throw new ArgumentException(nameof(username), "Username cannot be null, empty, or whitespace");
            }
